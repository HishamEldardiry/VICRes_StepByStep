...
C************************************************************************************************************************************************************************************
     ...
     &  ICOL, IROW, RPATH, NO_STAS,RES_DIRECT,RES_EVAPORATION,NO_OF_BOX, NUMRES,STEPBYSTEP,NDAY_SIM,SIM_YEAR, SIM_MON, SIM_DAY)
      IMPLICIT NONE
c     Declare variables
c     RCS ID STRING
      INTEGER       NUMRES,NDAY_SIM,SIM_YEAR, SIM_MON, SIM_DAY
      INTEGER       IROW, ICOL
...
c     Subroutine main body
      ...
      IF (STEPBYSTEP) THEN
            ...
            print*, NDAY_SIM,SIM_YEAR, SIM_MON, SIM_DAY,FLOW(NDAY_SIM)
            ...
            ELSE
                  ...
                  print*, SIM_YEAR, SIM_MON, SIM_DAY,FLOW(NDAY_SIM)
                  WRITE(30,*) SIM_YEAR, SIM_MON, SIM_DAY,FLOW(NDAY_SIM)    
                  WRITE(31,*) SIM_YEAR, SIM_MON, SIM_DAY,FLOW(NDAY_SIM) / FACTOR_SUM 
            END IF
            ...
            CLOSE(31)
      ELSE      
            print*, 'writing data for batch mode with all steps...',NAME5                
            CLEN = INDEX(OUTPATH,' ')-1
...
c     Write reservoirs output of the last station
     ...
     & (FLOW, DAYS, NAME5, FACTOR_SUM, OUTPATH,IDAY,IMONTH,IYEAR,
     &  VOL, FLOWIN, FLOWOUT, HHO, HYDROPOWER,HTK, RESER, NCOL,NROW,
     &  ICOL, IROW, RPATH, NO_STAS,RES_DIRECT,RES_EVAPORATION,NO_OF_BOX,VRESER, NUMRES,STEPBYSTEP,NDAY_SIM)
      IMPLICIT NONE
c     Declare variables
c     RCS ID STRING
      INTEGER       NREACH, NUMRES,NDAY_SIM,STEPS
      INTEGER       IROW, ICOL
      INTEGER       NO_OF_BOX(NUMRES)
      INTEGER       DAYS
      INTEGER       NROW, NCOL
