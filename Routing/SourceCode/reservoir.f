...
C       Look for starting year
        ...
        ! Added by Hisham to avoid using values from previous time step when having headwater cell (no upstream cells and so no convolution needed)
...
C************************************************************************************************************************************************************************************
     ...
     &  XNE, YNE, NO_STAS, RES_DIRECT, RES_EVAPORATION, TRVLTIME,RESORDER,NAMERS5,NAME5,NREACH,UH_SS,VRESER,NUMRES, NSTATIONS,STEPBYSTEP,NDAY_SIM,OUTPATH)
        IMPLICIT NONE
c       Declare variables
        INTEGER     N, DAYS, NDAY,NDAY_SIM, START_YEAR,START_MO,NREACH,NUMRES, NSTATIONS,PREV_DAY,PREV_MON,PREV_YEAR,STEPS,CLEN
        INTEGER     NCOL,NROW,ICOL,PMAX,KE,UH_DAY
        ...
        INTEGER     RULE(NUMRES,NREACH), WITHIRRIGATION(NUMRES,NREACH),BTHMET_ACTIVE(NUMRES,NREACH)
        ...
        REAL        BTHMET_A(NUMRES,NREACH),BTHMET_B(NUMRES,NREACH),BTHMET_C(NUMRES,NREACH),BTHMET_D(NUMRES,NREACH)
...
c       Look for reservoirs sequences
        ...
        DO W=1,NSTATIONS
            ...
            print*, 'Number of Reservoirs', NO_STAS(W)
            WRITE(WTEMP, 10) W
...
10          FORMAT(I4)
     ...
     &                   RES_EVAPORATION, NO_STAS(W),NUMRES)   
             ...
             IF (STEPBYSTEP) THEN
               ...
               IF (NDAY_SIM.GT.1) THEN   
                  ...
                  storage_filename='RESERVOIR_VOL_STATION'//trim(adjustl(W_STR))//'_STEP'//trim(adjustl(NDAY_SIM_STR))//'.txt'
                   ...
                   READ(99, *) (VOL(J,1,W), J = 1, NO_STAS(W))
...
c            !!!!!!!!!!!!Read reservoir characteristics/parameters from input file (/Reservoirs/res1.txt, /Reservoirs/res2.txt,...)
...
c               H0(J,W) = (HRESERMAX(J,W)-HRESERMIN(J,W)*(VRESERTHAT(J,W)+VDEAD(J,W))/VDEAD(J,W))/(1-(VRESERTHAT(J,W)+VDEAD(J,W))/VDEAD(J,W))    !This actually performs better, but we will fix the problem of H0 with reservoirs bathimetry
                ENDIF
                ...
                ELSE 
                    VOL(J,1,W) = 0.001
                END IF
            END DO   ! End of J Loop for N_STATS [Line 843]
            
        END DO        ! End of W Loop for NSTATIONS [Line 758]
        
c     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! LOOP OVER STATIONS AND  RESERVOIRS FOR OPERATING CURVE
...
c               Check the neccesity to spill water
     ...
     &          (( HHO(J,I)+HHO(J,I+1))/2-(HRESERMAX(J,W)-REALHEAD(J,W)))/1000			! this part is for hydropower production estimation, ignore if work with irrigation reservoirs
                ...
                IF (RES_DIRECT(J,2,W) .EQ. 0) THEN  ! No downstream reservoirs
                    RESFLOWS(NO_STAS(W),II+1+INT(TRVLTIME(J)),W) = RESFLOWS(NO_STAS(W),II+1+INT(TRVLTIME(J)),W) + FLOWOUT(J,I,W)
     ...
     &!  'FLOWOUT',FLOWOUT(J,I),' VOL ',VOL(J,I),' ELEC', ENERGYPRO(J,I)
            ...
            END DO     !!! END OF J LOOP OVER RESERVOIRS of STATION [W] 
                ...
                IF (RESFLOWS(NO_STAS(W),II,W) .LT. 0) THEN
                     RESFLOWS(NO_STAS(W),II,W) = 0
                END IF
                FLOW(II,W) = RESFLOWS(NO_STAS(W),II,W)   ! change I(step) to II(simulated_day) 
                
                
                !!!!  Save Reservoir States for STEPBYSTEP Version
                IF (STEPBYSTEP) THEN
                   write(W_STR,10) W ! convert integer to string to be able to concatenate in filename with strings
                   write(NDAY_SIM_STR,10) (NDAY_SIM)
                   write(NDAY_SIM_STR_NEXT,10) (NDAY_SIM+1)
                   
                ! Output Path for StepByStep Version
...
c          !SAVE RESFLOWS TO READ THEM LATER for StepByStep Version 
                    ...
                    END DO
                    !print*, 'Saving Reservoir Flow:',RESFLOWSTRING_SIM
                    close(73)
                   ...
                   print*, 'Storage Filename:',storage_filename
                ...
                !    res_filename='RESERVOIR_STATES_STATION_'//trim(adjustl(W_STR))//'_STEP_'//trim(adjustl(NDAY_SIM_STR))//'.txt'      ! needed for coupling
                !    open(77, file=OUTPATH(1:CLEN)//'/STEPBYSTEP/'//res_filename)
                !    DO J = 1, NO_STAS(W)-1
                !       WRITE(77, *)  'DATE',CRTDATE,'Reservoir',J,'INFLOW',FLOWIN(J,I),'RELEASE',FLOWOUT(J,I,W),'STORAGE',VOL(J,I,W),'HYDROPOWER', ENERGYPRO(J,I) 
                !    END DO
                !    close(77)
